...
export class AutomationOrchestrator {
    ...
    async run(): Promise<void> {
        ...
        try {
            ...
            // Step 6: Verify selections before executing prompt
            const pg = this.browser.getPage();
            const hasLevel = await pg.locator(`text=${this.config.prompt.educationLevel}`).count();
            const hasSubject = await pg.locator(`text=${this.config.prompt.subject}`).count();
            if (!hasLevel || !hasSubject) {
                throw new Error('Education level or subject value missing after prompt creation');
            }
            
            // Step 6: Run prompt (abort if failure)
            const ran = await this.promptCreator.runPrompt();
            if (!ran) {
                throw new Error('Prompt execution failed, aborting automation');
            }
            ...
            if (frontierEnabled) {
                ...
                // allow up to 10 minutes for frontier generation
                const frontierReady = await this.workbenchOrchestrator!.testOnFrontierModels(5, 600000);
                if (frontierReady) {
