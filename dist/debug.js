...
async function debug() {
    ...
    const browser = new browser_manager_1.BrowserManager(config.screenshotDir);
    const authenticator = new authenticator_1.Authenticator(browser);
    const projectSelector = new project_selector_1.ProjectSelector(browser);
    try {
        // Launch and login
        await browser.launch(config.headless);
        await authenticator.login(config.credentials, config.project.baseUrl);
        await projectSelector.navigateToProject(config.project.projectName, config.project.baseUrl, config.project.projectUrl);
