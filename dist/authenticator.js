...
class Authenticator {
    ...
    async login(credentials, baseUrl) {
        ...
        try {
            ...
            const emailFilled = await this.browser.fill('input[type="email"]||input[name="email"]||input#email||input[placeholder*="email"]', credentials.email, 2000);
            if (!emailFilled)
                throw new Error('Email input not found');
            // Fill password
            const passwordFilled = await this.browser.fill('input[type="password"]||input[name="password"]||input#password', credentials.password, 2000);
            if (!passwordFilled)
                throw new Error('Password input not found');
            // Click submit button
            const submitted = await this.browser.click('button[type="submit"]||button:has-text("Login")||button:has-text("Sign in")||button:has-text("Sign In")', 2000);
            if (!submitted)
